...
## 查看业务部署和POD状况
1. 确认服务的调用链路： 网关服务fe-gateway通过域名 fe.xxx.com调用了git-api-service服务
1. fe-gateway和git-api-service没有发布记录
2. Pod运行正常
...
```
2. 在查看git-api-service服务日志，没有日志记录


由1,2可分析网关请求没有发送到服务，有可能由很多原因，大概率是运维问题。
3. 从网关的pod ping 服务的pod，不通
bash-4.4# ping 100.98.164.174
PING 100.98.164.174 (100.98.164.174): 56 data bytes

4. 当网关pod ping通了服务pod之后，网关的日志正常了
...
```
6. 开始在git-api-service curl git服务，发现时好时坏
```
...
```
7. 总结下一共发现了2个问题，需要反馈给运维

A：网关有时候ping不通服务pod
B：git服务pod curl git请求时好时坏